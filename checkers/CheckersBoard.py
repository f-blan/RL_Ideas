...
class CheckersBoard:
    ...
    def _white_moves(self) -> int:
        not_occ = ~(self.W | self.B)&0xFFFFFFFF
        w_k = self.W&self.K
        w = self.W ^ self.K
    ...
    def _black_jumps(self) -> int:
        ...
        tmp = ((W_R5R&not_occ)>>3)&self.W
        movers|= (W_L3&b)&(tmp>>4)

        tmp = ((W_R3L&not_occ)>>5)&self.W
        movers|= (W_R3&b)&(tmp>>4)

        tmp = (self.W << 4)&not_occ
        moves |= ((W_L3R&b)<<9)&tmp
        moves |= ((W_R3L&b)<<7)&tmp

        tmp = ((W_R3L&self.W)<<3) & not_occ
        moves |= ((b&W_L3)<<7) &tmp

        tmp = ((W_L3R&self.W)<<5) &not_occ
        moves |= ((b&W_R3)<<9) & tmp
        ...
        if b_k:
            ...
            tmp = ((W_L5L&not_occ)<<3)&self.W
            k_movers|= (W_R3&b_k)&(tmp<<4)

            tmp = ((W_L3R&not_occ)<<5)&self.W
            ...
            tmp = ((W_L3R&self.W)>>3) & not_occ
            k_moves |= ((b_k&W_R3)>>7) &tmp

            tmp = ((W_R3L&self.W)>>5) &not_occ
